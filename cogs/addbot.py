...
class AddBot(commands.Cog):
    ...
    async def addbot(self,ctx,id=None,*,desc=None):
        ...
        try:
            ...
            await chan.send(content=f'<@&{config.mention_on_submit_role_id}> New Invite Request!',embed=emb)
    ...
    async def reject(self,ctx):
        ...
        try:
            ...
            if int(emb_dict['footer']['text'].split('|')[0].replace(' ','')) in config.submitted_bots:
                config.submitted_bots.remove(int(emb_dict['footer']['text'].split('|')[0].replace(' ','')))
            return await ctx.reply(f"{emb_dict['footer']['text'].split('|')[0].replace(' ','')} has been rejected successfully!")
        except Exception as e:
            return await ctx.send(embed=discord.Embed(description='Something went wrong: '+str(e)))
